...
BASE_URL = 'https://www.winemag.com/?s=&drink_type=spirit&page={0}'
...
class Scraper():
    ...
    def scrape_page(self, page_url, isolated_review_count=0, retry_count=0):
        scrape_data = []
        try:
            response = self.session.get(page_url, headers=HEADERS,allow_redirects=False)
        except:
            retry_count += 1
            if retry_count <= 3:
                self.session = requests.Session()
